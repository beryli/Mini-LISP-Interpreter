...
def eval (var, body):
    ...
    # print(body)
    # assignment
    body = ' '.join([str(elem) for elem in body])
    body = "ret" + body
    # print(body)
    yacc.parse(body)
...
# ---------------- 8 named function
def p_lambdaName(p):
    'statement : LPAREN PN lambda_name RPAREN'
...
def p_expression_defFunc(p):
    '''lambda_name  : LPAREN ID para RPAREN
    ...
    body = list(chain.from_iterable(names[p[2]][1]))
    # print(local)
    # print(body)
    eval(local, body)
    p[0] = 99
...
def p_lambda(p):
    'statement : LPAREN PN lambda_call RPAREN'

def p_expression_lambda(p):
    'lambda_call : LPAREN LPAREN Lambda para var RPAREN items RPAREN'
    # p[0] = p[3] # [args body]
    # print(p[4])
...
def p_body(p):
    ...
    p[0] = list(chain.from_iterable(p[1:]))
...
def p_var_multiply(p):
    ...
    for idx, val in enumerate(p[0]):
        if isinstance(val, int):
            p[0][idx] = str(val)
    p[0] = list(chain.from_iterable(p[0]))
